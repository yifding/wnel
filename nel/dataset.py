...
#            else:
#                try:
#                    tok, _, _, cat  = line.split()
#                    _, cat = cat.split('-')
#                    if cat != prv_cat:
#                        if prv_cat != '':
#                            cur_doc['mentions'].append(prv_m)
#                            cur_doc['cat'].append(prv_cat)
#                        prv_m = ''
#                        prv_cat = ''
#
#                        if cat != 'O':
...
def eval(testset, system_pred, cats=None):
    gold = []
    pred = []
    org_cats = cats
    if org_cats is None:
    ...
    true_pos = 0
    for g, p, c in zip(gold, pred, cats):
...
if __name__ == "__main__":
    ...
    # conll_path = '/disk/scratch1/ple/workspace/corpora/NYT/nyt_eng_200504.el.txt'
    # cand_path = '/disk/scratch1/ple/workspace/corpora/NYT/nyt_eng_200504.el.txt.csv'
    # data = CoNLLDataset.load_file(conll_path, cand_path, person_path)
